var data = require("./data.json");

var headToHead = {
  "Ryan Coxe": {
    opponents: {
      "Kevin Dobkin": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Jason Knaak": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "JD Langefeld": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Reece Langerock": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Jimmy Ouska": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Sean Quill": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Matt Reschke": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Bryan Steger": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Mike Unverricht": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Trey Ward": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Alex Warner": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      }
    }
  },
  "Kevin Dobkin": {
    opponents: {
      "Ryan Coxe": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Jason Knaak": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "JD Langefeld": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Reece Langerock": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Jimmy Ouska": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Sean Quill": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Matt Reschke": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Bryan Steger": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Mike Unverricht": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Trey Ward": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Alex Warner": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      }
    }
  },
  "Jason Knaak": {
    opponents: {
      "Ryan Coxe": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Kevin Dobkin": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "JD Langefeld": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Reece Langerock": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Jimmy Ouska": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Sean Quill": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Matt Reschke": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Bryan Steger": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Mike Unverricht": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Trey Ward": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Alex Warner": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      }
    }
  },
  "JD Langefeld": {
    opponents: {
      "Ryan Coxe": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Kevin Dobkin": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Jason Knaak": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Reece Langerock": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Jimmy Ouska": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Sean Quill": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Matt Reschke": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Bryan Steger": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Mike Unverricht": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Trey Ward": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Alex Warner": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      }
    }
  },
  "Reece Langerock": {
    opponents: {
      "Ryan Coxe": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Kevin Dobkin": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Jason Knaak": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "JD Langefeld": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Jimmy Ouska": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Sean Quill": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Matt Reschke": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Bryan Steger": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Mike Unverricht": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Trey Ward": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Alex Warner": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      }
    }
  },
  "Jimmy Ouska": {
    opponents: {
      "Ryan Coxe": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Kevin Dobkin": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Jason Knaak": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "JD Langefeld": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Reece Langerock": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Sean Quill": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Matt Reschke": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Bryan Steger": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Mike Unverricht": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Trey Ward": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Alex Warner": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      }
    }
  },
  "Sean Quill": {
    opponents: {
      "Ryan Coxe": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Kevin Dobkin": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Jason Knaak": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "JD Langefeld": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Reece Langerock": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Jimmy Ouska": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Matt Reschke": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Bryan Steger": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Mike Unverricht": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Trey Ward": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Alex Warner": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      }
    }
  },
  "Matt Reschke": {
    opponents: {
      "Ryan Coxe": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Kevin Dobkin": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Jason Knaak": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "JD Langefeld": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Reece Langerock": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Jimmy Ouska": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Sean Quill": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Bryan Steger": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Mike Unverricht": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Trey Ward": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Alex Warner": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      }
    }
  },
  "Bryan Steger": {
    opponents: {
      "Ryan Coxe": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Kevin Dobkin": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Jason Knaak": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "JD Langefeld": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Reece Langerock": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Jimmy Ouska": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Sean Quill": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Matt Reschke": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Mike Unverricht": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Trey Ward": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Alex Warner": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      }
    }
  },
  "Mike Unverricht": {
    opponents: {
      "Ryan Coxe": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Kevin Dobkin": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Jason Knaak": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "JD Langefeld": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Reece Langerock": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Jimmy Ouska": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Sean Quill": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Matt Reschke": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Bryan Steger": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Trey Ward": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Alex Warner": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      }
    }
  },
  "Trey Ward": {
    opponents: {
      "Ryan Coxe": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Kevin Dobkin": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Jason Knaak": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "JD Langefeld": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Reece Langerock": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Jimmy Ouska": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Sean Quill": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Matt Reschke": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Bryan Steger": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Mike Unverricht": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Alex Warner": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      }
    }
  },
  "Alex Warner": {
    opponents: {
      "Ryan Coxe": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Kevin Dobkin": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Jason Knaak": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "JD Langefeld": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Reece Langerock": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Jimmy Ouska": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Sean Quill": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Matt Reschke": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Bryan Steger": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Mike Unverricht": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      },
      "Trey Ward": {
        wins: 0,
        losses: 0,
        "points-scored-array": [],
        "points-allowed-array": []
      }
    }
  }
};

module.exports = {
  clear() {
    headToHead = {
      "Ryan Coxe": {
        opponents: {
          "Kevin Dobkin": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Jason Knaak": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "JD Langefeld": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Reece Langerock": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Jimmy Ouska": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Sean Quill": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Matt Reschke": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Bryan Steger": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Mike Unverricht": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Trey Ward": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Alex Warner": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          }
        }
      },
      "Kevin Dobkin": {
        opponents: {
          "Ryan Coxe": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Jason Knaak": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "JD Langefeld": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Reece Langerock": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Jimmy Ouska": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Sean Quill": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Matt Reschke": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Bryan Steger": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Mike Unverricht": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Trey Ward": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Alex Warner": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          }
        }
      },
      "Jason Knaak": {
        opponents: {
          "Ryan Coxe": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Kevin Dobkin": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "JD Langefeld": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Reece Langerock": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Jimmy Ouska": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Sean Quill": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Matt Reschke": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Bryan Steger": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Mike Unverricht": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Trey Ward": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Alex Warner": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          }
        }
      },
      "JD Langefeld": {
        opponents: {
          "Ryan Coxe": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Kevin Dobkin": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Jason Knaak": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Reece Langerock": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Jimmy Ouska": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Sean Quill": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Matt Reschke": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Bryan Steger": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Mike Unverricht": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Trey Ward": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Alex Warner": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          }
        }
      },
      "Reece Langerock": {
        opponents: {
          "Ryan Coxe": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Kevin Dobkin": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Jason Knaak": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "JD Langefeld": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Jimmy Ouska": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Sean Quill": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Matt Reschke": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Bryan Steger": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Mike Unverricht": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Trey Ward": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Alex Warner": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          }
        }
      },
      "Jimmy Ouska": {
        opponents: {
          "Ryan Coxe": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Kevin Dobkin": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Jason Knaak": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "JD Langefeld": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Reece Langerock": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Sean Quill": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Matt Reschke": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Bryan Steger": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Mike Unverricht": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Trey Ward": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Alex Warner": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          }
        }
      },
      "Sean Quill": {
        opponents: {
          "Ryan Coxe": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Kevin Dobkin": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Jason Knaak": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "JD Langefeld": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Reece Langerock": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Jimmy Ouska": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Matt Reschke": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Bryan Steger": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Mike Unverricht": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Trey Ward": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Alex Warner": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          }
        }
      },
      "Matt Reschke": {
        opponents: {
          "Ryan Coxe": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Kevin Dobkin": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Jason Knaak": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "JD Langefeld": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Reece Langerock": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Jimmy Ouska": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Sean Quill": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Bryan Steger": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Mike Unverricht": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Trey Ward": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Alex Warner": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          }
        }
      },
      "Bryan Steger": {
        opponents: {
          "Ryan Coxe": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Kevin Dobkin": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Jason Knaak": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "JD Langefeld": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Reece Langerock": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Jimmy Ouska": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Sean Quill": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Matt Reschke": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Mike Unverricht": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Trey Ward": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Alex Warner": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          }
        }
      },
      "Mike Unverricht": {
        opponents: {
          "Ryan Coxe": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Kevin Dobkin": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Jason Knaak": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "JD Langefeld": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Reece Langerock": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Jimmy Ouska": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Sean Quill": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Matt Reschke": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Bryan Steger": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Trey Ward": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Alex Warner": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          }
        }
      },
      "Trey Ward": {
        opponents: {
          "Ryan Coxe": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Kevin Dobkin": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Jason Knaak": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "JD Langefeld": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Reece Langerock": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Jimmy Ouska": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Sean Quill": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Matt Reschke": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Bryan Steger": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Mike Unverricht": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Alex Warner": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          }
        }
      },
      "Alex Warner": {
        opponents: {
          "Ryan Coxe": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Kevin Dobkin": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Jason Knaak": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "JD Langefeld": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Reece Langerock": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Jimmy Ouska": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Sean Quill": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Matt Reschke": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Bryan Steger": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Mike Unverricht": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          },
          "Trey Ward": {
            wins: 0,
            losses: 0,
            "points-scored-array": [],
            "points-allowed-array": []
          }
        }
      }
    };
  },
  
  calculateHeadToHead(weekData, season, user, regularOrPlayoffs) {
    if (headToHead[user].opponents[weekData.opponent]) {

      headToHead[user].opponents[weekData.opponent]["points-scored-array"].push({ score: weekData["points-scored"], date: `Week ${weekData['week-number']}, ${season + 2012}` });
      headToHead[user].opponents[weekData.opponent]["points-allowed-array"].push({ score: weekData["points-allowed"], date: `Week ${weekData['week-number']}, ${season + 2012}` });

      if (weekData["points-scored"] > weekData["points-allowed"]) {
        headToHead[user].opponents[weekData.opponent].wins++;
      } else if (weekData["points-scored"] < weekData["points-allowed"]) {
        headToHead[user].opponents[weekData.opponent].losses++;
      }
    }
  },

  run(
    includeRegularSeason,
    includePlayoffs,
    seasons = [0, 1, 2, 3, 4],
    selectedUsers = [
      "Ryan Coxe",
      "Kevin Dobkin",
      "Jason Knaak",
      "JD Langefeld",
      "Reece Langerock",
      "Jimmy Ouska",
      "Sean Quill",
      "Matt Reschke",
      "Bryan Steger",
      "Mike Unverricht",
      "Trey Ward",
      "Alex Warner"
    ]
  ) {
    var that = this;
    that.clear();
    return new Promise(function(resolve, reject) {
      var uI = 0; //userIndex
      var sI = 0; //seasonIndex
      var rI = 0; //regularSeasonWeekIndex
      var pI = 0; //playoffWeekIndex
      var rS = "regular season"; //regular season  string
      var pS = "playoffs"; //playoffs string

      for (uI = 0; uI < data.users.length; uI++) {
        var userName = data.users[uI].name;

        if (selectedUsers.includes(userName)) {
          for (sI = 0; sI < data.users[uI].season.length - 1; sI++) {
            if (seasons.includes(sI)) {
              if (includeRegularSeason) {
                for (rI = 0; rI < data.users[uI].season[sI][rS].length; rI++) {
                  let dataToCheck = data.users[uI].season[sI][rS][rI];

                  that.calculateHeadToHead(dataToCheck, sI, userName, "R");
                }
              }
              if (includePlayoffs) {
                for (pI = 0; pI < data.users[uI].season[sI][pS].length; pI++) {
                  let dataToCheck = data.users[uI].season[sI][pS][pI];
                  that.calculateHeadToHead(dataToCheck, sI, userName, "P");
                }
              }
            }
          }
        }
      }

      resolve({
        headToHead,
        completed: true
      });
    });
  }
};
